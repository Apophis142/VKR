...
if not os.path.exists("models/training/graphics"):
    os.makedirs("models/training/graphics")
if not os.path.exists("models/training/loss_archives"):
    os.makedirs("models/training/loss_archives")
if not os.path.exists("weights/training"):
    os.makedirs("weights/training")
...
def train_nn(
    ...
    batch_loader = {
        "OneSequenceProcessing": load_batch_sequence_frames,
        "OneFramePairProcessing": load_batch_pair_frames,
    }[net.name()]
...
def training_thread(
    ...
    with tqdm(total=(num_train_batches + num_test_batches) * num_epochs, position=0, leave=True) as pbar:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % epoch_frequency_save == 0 and epoch:
                ...
                with open(
                        f"models/training/loss_archives/{filename_to_save}_loss_history_epoch_{epoch}_training.pkl",
                        'wb+'
                ) as f:
    ...
    with open(
            f"models/training/loss_archives/{filename_to_save}_loss_history_epoch_{epoch}.pkl",
            'wb+'
    ) as f:
        pickle.dump((tuple(train_hist), tuple(eval_hist)), f)
    ...
    plt.title("Loss")
    plt.legend()
    plt.savefig(fname=f"models/training/graphics/{filename_to_save}.pdf")
